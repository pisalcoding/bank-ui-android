package pisal.me.learn.bankui.common.blur;
...
public class BlurLayout extends FrameLayout {
    ...
    // Tested many times and this number is a good tradeoff
    public static final float DEFAULT_DOWNSCALE_FACTOR = 0.12f;

    // Somewhere between 4 -> 20
    // The higher the blur, the slower the UI
    public static final int DEFAULT_BLUR_RADIUS = 16;

    // Somewhere between 4 -> 30
    // The higher the FPS, the more resources consumed
    public static final int DEFAULT_FP4S = 8;
    ...
    // Blur radius passed directly to stack blur library.
    private int mBlurRadius;

    // Number of blur invalidation to do per second.
    private int mFPS;
    ...
    public BlurLayout(Context context, AttributeSet attrs) {
        ...
        try {
            ...
            mFPS = a.getInteger(R.styleable.BlurLayout_fps, DEFAULT_FP4S);
        } finally {
            a.recycle();
        }
    ...
    private final Choreographer.FrameCallback invalidationLoop = new Choreographer.FrameCallback() {
        ...
        public void doFrame(long frameTimeNanos) {
            invalidate();
            Choreographer.getInstance().postFrameCallbackDelayed(this, 4000 / mFPS);
        }
    };
    ...
    private Bitmap blur() {
        ...
        bitmap = BlurKit.Companion.getInstance().blur(bitmap, mBlurRadius);
